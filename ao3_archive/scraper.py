...
def scrape_work(soup, link):
    ...
    work_ID = "AO3W" + (link.split('/')[4].strip())
...
def main():
    ...
    work_results = []
    series_results = []
    with open('fic_links.txt', 'r') as file:
        for line in file:
    ...
    response = requests.get(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36'})
